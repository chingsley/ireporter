...
class Validate {
  static async newRedflag(req, res, next) {
    ...
    const {
      id, type, location, comment,
    } = req.body;

    const missingFields = [id, type, location, comment].map((field, index) => {
      const keys = {
        0: 'id',
        1: 'type',
        2: 'location',
    ...
    if (!id || !type || !location || !comment) return response(`values are required for the following fields: ${missingFields}`);
    ...
    if(!Helper.isValidId((id.trim()))) return response(`${id} is not a valid id for any user`);
    ...
    const matchingUser = users.filter(user => Number(user.id) === Number((id.trim())));
    ...
    req.newRedflag.createdBy = Number((id.trim()));
  ...
  static async editRedflagLocation(req, res, next) {
    ...
    if(redflagToEdit[0].createdBy !== Number(userId.trim())) return res.status(401).json({
      status: 401,
  ...
  static async editRedflagComment(req, res, next) {
    ...
    // return 401 if the redflag exists, but does not belong to the user trying to access it
    if (redflagToEdit[0].createdBy !== Number(userId.toString().trim())) return res.status(401).json({
      status: 401,
      error: `you have no authorization to edit that redfag`
    });
  ...
  static async deleteRedflag(req, res, next) {
    ...
    if (Number(redflagToDelete[0].createdBy) !== Number(userId.toString().trim())) return res.status(401).json({
      status: 401,
